...
void pagingInit(){
    ...
    for(uint64 x=allocPage();x<maxMemory;x++){
        char *ptr = (char*)x;
        *ptr = 'v';
    }
    
    
    /*for(uint64 p=1;p<12;p++){
      setPagingL1(0,0,p,getNextPagingKernel(),PRESENT|RW|WRITE_THROUGH|USER);
      setPaging(0,0,p,0,p*2*MB,PRESENT|RW|WRITE_THROUGH|USER,512);
    } // Setup paggin to 2MB at 128MB
    */
}
